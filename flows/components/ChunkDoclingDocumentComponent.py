...
from lfx.io import BoolInput, DropdownInput, HandleInput, MessageTextInput, Output, StrInput
...
class ChunkDoclingDocumentComponent(Component):
    ...
    def chunk_documents(self) -> DataFrame:
        ...
        if self.chunker == "HybridChunker":
            ...
            max_tokens: int | None = int(self.max_tokens) if self.max_tokens else None
            if self.provider == "Hugging Face":
                try:
                    ...
                    raise ImportError(msg) from e
                logger.info(f"Creating a tokenizer for model {self.hf_model_name}, with max_tokens {max_tokens}.")
...
class ChunkingSerializerProviderWithMarkdownTables(BaseSerializerProvider):
    """Serializer provider used for chunking purposes."""

    @override
    def get_serializer(self, doc: DoclingDocument) -> BaseDocSerializer:
        """Get the associated serializer."""
        return ChunkingDocSerializerWithMarkdownTables(doc=doc)